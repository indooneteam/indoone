...
$('moreBtn').addEventListener('click',()=>openModal(`<div class="modal-head"><h2>More Options</h2><button type="button" class="close-btn" data-close>×</button></div><button type="button" class="settings-row" data-sort-modal><span>Sort accounts<small>${indooneState.newestFirst?'Newest first':'Name A–Z'}</small></span><b>›</b></button><button type="button" class="settings-row" data-security><span>Security<small>Review privacy settings</small></span><b>›</b></button><button type="button" class="settings-row" data-about><span>About Indoone<small>Version 1.0.0</small></span><b>›</b></button>`));
