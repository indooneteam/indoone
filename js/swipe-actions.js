(() => {
  ...
  async function runAction(wrap, direction) {
    const id = Number(wrap.querySelector('.account')?.dataset.id);
    if (!id) return;
    wrap.classList.remove('swiping-left', 'swiping-right');
    wrap.querySelector('.account')?.classList.add('swipe-success');
    try {
      if (direction === 'left') {
        await window.toggleFavorite(id);
      } else {
        const account = window.indooneState?.accounts?.find(a => Number(a.id) === id);
        if (!account) return;
        await window.IndooneCloudAccounts.moveToTrash(account);
        const index = window.indooneState.accounts.findIndex(a => Number(a.id) === id);
        if (index >= 0) window.indooneState.accounts.splice(index, 1);
  ...
  function reset(wrap) {
    if (!wrap) return;
    wrap.classList.remove('swiping-left', 'swiping-right');
    wrap.querySelector('.account')?.classList.remove('swipe-active', 'swipe-success');
  }
  ...
  document.addEventListener('touchend', async event => {
    ...
    const direction = dx < -THRESHOLD ? 'left' : dx > THRESHOLD ? 'right' : '';
