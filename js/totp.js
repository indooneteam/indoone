const TOTP = (() => {
  ...
  function parseOtpAuth(uri) {
    ...
    if (url.protocol !== 'otpauth:' || url.host !== 'totp') throw new Error('Unsupported OTP URI');
  ...
  async function generate(secret, counter, digits = 6, algorithm = 'SHA1') {
  ...
  return {generate, parseOtpAuth, base32ToBytes};
})();
window.TOTP = TOTP;
