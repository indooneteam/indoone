window.IndooneRecoveryPdf = (() => {
  ...
  async function makePayload(accounts,pin){
    ...
    const clear=enc.encode(JSON.stringify({v:1,accounts,createdAt:new Date().toISOString()}));
  ...
  function buildPdf(payload){
    const lines=['INDOONE AUTHENTICATOR','ENCRYPTED RECOVERY FILE','PIN REQUIRED TO RESTORE ACCOUNTS','RECOVERY_PAYLOAD_BEGIN'];
    for(let i=0;i<payload.length;i+=90) lines.push(payload.slice(i,i+90));
    lines.push('RECOVERY_PAYLOAD_END');
    let stream='BT /F1 9 Tf 40 800 Td 12 TL ';
    lines.forEach((line,i)=>{stream+=`(${esc(line)}) Tj`; if(i<lines.length-1) stream+=' T* ';});
    stream+=' ET';
    const objects=[];
  ...
  async function create(accounts,pin){ const payload=await makePayload(accounts,pin); localStorage.setItem(KEY,JSON.stringify({createdAt:new Date().toISOString()})); download(buildPdf(payload)); return true; }
  function isReady(){return Boolean(localStorage.getItem(KEY));}
  function clearReady(){localStorage.removeItem(KEY);}
  function extractPayload(text){ const m=text.match(/RECOVERY_PAYLOAD_BEGIN([\s\S]*?)RECOVERY_PAYLOAD_END/); if(!m) throw new Error('Recovery payload not found'); return JSON.parse(m[1].replace(/[\r\n ]/g,'')); }
  async function restoreFromFile(file,pin){ const text=new TextDecoder().decode(await file.arrayBuffer()); return openPayload(extractPayload(text),pin); }
  return {create,isReady,clearReady,restoreFromFile};
})();
