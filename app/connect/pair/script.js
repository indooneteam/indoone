(() => {
  ...
  async function loadMarkup() {
    ...
    markupPromise = fetch(
      ...
      .then(response => {
        if (!response.ok) {
          throw new Error('Pair page could not be loaded.');
        }
  ...
  async function show({ remember = true } = {}) {
    ...
    if (remember) {
      window.IndoonePageState?.set('pair');
    }
    ...
    try {
      mount
        .querySelector('.connect-pair-page')
        ?.remove();
      ...
      bind(
        mount.querySelector('.connect-pair-page')
      );
    } catch (error) {
      ...
      mount.insertAdjacentHTML(
        'beforeend',
        `<section class="connect-pair-page">
          <button
            type="button"
            class="connect-back"
            data-pair-action="home"
          >
            <svg
              viewBox="0 0 24 24"
              aria-hidden="true"
          ...
          <p class="connect-pair-copy">
            Pair page could not be loaded.
          </p>
        </section>`
      );
