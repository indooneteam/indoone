(() => {
  ...
  function loadDraft(endpointId) {
    if (drafts.has(endpointId)) {
      return drafts.get(endpointId);
    }
  ...
  function sendPermissions(endpointId, permissions) {
    ...
    try {
      window.IndooneNative?.sendNearbyText?.(
        endpointId,
        message
      );
    } catch (_) {}
  ...
  function openPermissionPage(
    ...
    if (!endpointId || openEndpoints.has(endpointId)) {
      return;
    }
    ...
    if (
      modal?.querySelector(
        '[data-live-permission-save], [data-bilateral-permission-save]'
      )
    ) {
      return;
    }
    ...
    window.openModal?.(`
      <section
        class="connect-feature connect-permission-feature"
        data-bilateral-permission="${escapeHtml(endpointId)}"
      >
        <div class="modal-head">
          ...
          <button
            type="button"
            class="close-btn"
            data-bilateral-permission-close
        ...
        <div class="permission-list">
          ${[
            ...
            <label class="permission-item">
              <input
                type="checkbox"
                data-bilateral-permission="${key}"
                ${existingDraft[key] ? 'checked' : ''}
              />
              <span>
                <b>
                  ${label}
                ...
                <small>
                  Allow access to approved ${label.toLowerCase()}.
                </small>
              </span>
            </label>
          `).join('')}
        ...
        <p
          class="connect-muted"
          data-bilateral-permission-status
        >
          Nearby connection is established. Choose what this device may access, then save.
        </p>

        <button
          type="button"
          class="primary"
          data-bilateral-permission-save
        >
          Allow selected
        </button>
        <button
          type="button"
          class="secondary"
          data-bilateral-permission-deny
        >
          Deny all
        </button>
      </section>
    ...
    currentModal
      .querySelector('[data-bilateral-permission-save]')
      ?.addEventListener('click', () => {
        const allowed = [
          ...currentModal.querySelectorAll(
            '[data-bilateral-permission]:checked'
          )
        ]
          .map(
            input => input.dataset.bilateralPermission
          )
          .filter(
            key => [
              'photos',
    ...
    currentModal
      .querySelector('[data-bilateral-permission-deny]')
      ?.addEventListener(
        'click',
        () => save([])
      );
    ...
    currentModal
      .querySelector('[data-bilateral-permission-close]')
      ?.addEventListener('click', () => {
        openEndpoints.delete(endpointId);
  ...
  window.addEventListener('indoone-nearby', event => {
    ...
    if (detail.type === 'connectionResult') {
      ...
      const meta = connectionMeta.get(endpointId) || {
        name: 'Nearby device',
        direction: 'outgoing'
      };
