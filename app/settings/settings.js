(() => {
  const SETTINGS = [
    { id: 'profile', title: 'Profile', subtitle: 'Email & mobile number', icon: 'profile', action: () => window.showProfile?.() },
    { id: 'app-lock', title: 'App Lock', subtitle: 'PIN', icon: 'lock', action: () => window.showAppLock?.() },
    { id: 'biometric', title: 'Biometric Unlock', subtitle: 'Fingerprint / device credential', icon: 'biometric', toggle: true },
    { id: 'auto-lock', title: 'Auto-Lock', icon: 'timer', subtitle: () => {
      const value = window.IndooneAutoLock?.minutes?.() ?? 0;
      return value ? `After ${value} minute${value === 1 ? '' : 's'}` : 'Never';
    }, action: () => window.showAutoLock?.() },
    { id: 'about', title: 'About Indoone', subtitle: 'Version 1.8', icon: 'info', action: () => window.showAboutSettings?.() }
  ];
  ...
  function rowMarkup(item, biometricOn) {
    ...
    if (item.toggle) {
      ...
      text.innerHTML = `<b>${item.title}</b><small>${subtitle}</small>`;

      const button = document.createElement('button');
      button.type = 'button';
      button.className = `toggle settings-toggle ${biometricOn ? 'on' : ''}`;
      button.id = 'biometricToggle';
      button.setAttribute('aria-label', 'Toggle biometric unlock');
    ...
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'settings-row settings-row-button settings-option-row';
  ...
  function hideSettingsPage() {
    document.getElementById('settingsContent')?.setAttribute('hidden', '');
  }
  ...
  function showSettingsPage() {
    ...
    document.getElementById('accountsNav')?.classList.remove('active');
    document.getElementById('lobbyNav')?.classList.remove('active');
    document.getElementById('connectNav')?.classList.remove('active');
  }
  ...
  function renderSettingsPage() {
    ...
    head.innerHTML = '<p class="eyebrow">PREFERENCES &amp; SECURITY</p><h1>Settings</h1><p>Manage your account, security and app preferences.</p>';
    page.appendChild(head);

    const sections = [
      { label: 'Account', items: [SETTINGS[0]] },
      { label: 'Security', items: [SETTINGS[1], SETTINGS[2], SETTINGS[3]] },
      { label: 'App', items: [SETTINGS[4]] }
    ];
    ...
    page.querySelectorAll('[data-settings-action="profile"]').forEach(button => {
      button.addEventListener('click', () => window.showProfile?.());
    });
    page.querySelectorAll('[data-settings-action="app-lock"]').forEach(button => {
      button.addEventListener('click', () => window.showAppLock?.());
    });
    page.querySelectorAll('[data-settings-action="auto-lock"]').forEach(button => {
      button.addEventListener('click', () => window.showAutoLock?.());
    });
    page.querySelectorAll('[data-settings-action="about"]').forEach(button => {
      button.addEventListener('click', () => window.showAboutSettings?.());
    });

    page.querySelector('#biometricToggle')?.addEventListener('click', event => toggleBiometricSetting(event, event.currentTarget));
    return page;
  }
  ...
  document.addEventListener('click', event => {
    ...
    if (target.closest('#settingsNav')) return;
    if (target.closest('#accountsNav, #lobbyNav, #connectNav, .drawer-item')) hideSettingsPage();
  });
})();
