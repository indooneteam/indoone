(() => {
  ...
  async function renderProfile() {
    ...
    if (phone) {
      phone.textContent = 'Mobile number not set';
    }
  ...
  window.showProfile = function ({ remember = true } = {}) {
    if (remember) {
      window.IndoonePageState?.set('profile');
    }
    ...
    void fetch(
      'app/settings/profile/index.html?v=20260904a',
      { cache: 'no-store' }
    )
      .then(response =>
        response.ok
      ...
      .then(html => {
        ...
        modal
          .querySelector(
            '[data-profile-action="mobile"]'
          )
          ?.addEventListener(
            'click',
            () => window.showChangeMobile?.()
          );

        modal
          .querySelector(
            '[data-profile-action="email"]'
          )
          ?.addEventListener(
            'click',
            () => window.showChangeEmail?.()
          );
