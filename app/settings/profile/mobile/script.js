window.showChangeMobile = function () {
  ...
  fetch('app/settings/profile/mobile/index.html?v=20260904c', { cache: 'no-store' })
    ...
    .then(html => {
      ...
      const normalizeMobile = window.IndooneFirebaseAuthBase?.normalizeMobile || (value => {
      ...
      const saveButton = document.getElementById('saveMobileNumber');

      if (!auth || !db || !user) {
        return toast('Please sign in again before changing your mobile number.');
      }
      ...
      saveButton?.addEventListener('click', async () => {
        ...
        try {
          ...
          for (const candidate of candidates) {
            const existingSnapshot = await db.ref(`mobileIndex/${encodeURIComponent(candidate)}`).once('value');
            const existing = existingSnapshot.val();
            if (existing?.uid && String(existing.uid) !== String(user.uid)) {
          ...
          const updates = {
            ...
            [`users/${user.uid}/profile/email`]: user.email || profile.email || '',
          ...
          if (oldMobile && oldMobile !== value) {
            updates[`mobileIndex/${encodeURIComponent(oldMobile)}`] = null;
          }
