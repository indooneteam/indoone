window.IndooneBiometric = (() => {
  ...
  function authenticateForUnlock(onSuccess, onFallback) {
    ...
    if (!enabled()) {
      return onFallback?.('Biometric unlock is not enabled');
    }
  ...
  function enableForCurrentVault(onSuccess, onFallback) {
    ...
    try {
      pin = IndooneSecureSession.getPin();
    } catch (_) {
      return onFallback?.('Unlock App Lock with PIN first');
    }
    ...
    const handler = event => {
      ...
      const saved =
        window.IndooneNative.saveBiometricSecret(pin);
