window.initMenuLogoutAllDevices = function () {
  ...
  modal.innerHTML = `<div class="modal-head"><h2>Log out on all devices</h2><button class="close-btn" data-close>×</button></div><p>This signs you out on this device. Remote session revocation requires server-side Firebase Admin support.</p><button type="button" class="primary" id="confirmAllDevicesLogout">Log out on all devices</button><button type="button" class="secondary" data-close>Cancel</button>`;
