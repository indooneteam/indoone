window.initMenuDeleteLocalData = function () {
  document.getElementById('modal').innerHTML = `<div class="modal-head"><h2>Delete local data?</h2><button class="close-btn" data-close>×</button></div><p>This removes Indoone data stored on this device, including the encrypted vault and local sign-in markers. Your Indoone account and cloud data will not be deleted.</p><button type="button" class="primary danger" id="confirmLocalDelete">Delete local data</button><button type="button" class="secondary" data-close>Cancel</button>`;
