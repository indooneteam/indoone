(() => {
  ...
  function normalizeMobileValue(input, prefix, rawValue = input?.value || '') {
    ...
    if (raw !== normalizedDigits) {
      ...
      const nextCursor = Math.min(normalizedDigits.length, digitsBeforeCursor);
      try {
  ...
  function bindMobileDefaults(root) {
    bindMobileInput(root.querySelector('#authIdentifier'), root.querySelector('#loginMobilePrefix'));
    bindMobileInput(root.querySelector('#signupMobile'), root.querySelector('#signupMobilePrefix'));
  }
  ...
  async function showForgotPassword() {
    if (!showShell('<div class="auth-loading">Loading…</div>')) return;
    const modal = $('modal');
    const root = modal?.querySelector('.auth-page');
    if (!root || !window.IndooneForgotPassword?.load) {
      setStatus('Forgot Password page is unavailable.', true);
      return;
    }
    try {
      ...
      console.error('Forgot Password page failed to load:', error);
      showShell('<div class="auth-copy"><p class="eyebrow">ACCOUNT RECOVERY</p><h1>Forgot password?</h1><p>Unable to load password recovery right now.</p></div><button type="button" class="secondary" id="forgotPasswordLoadBack">Back to Login</button>');
      $('forgotPasswordLoadBack')?.addEventListener('click', showLogin);
    }
  }
  ...
  window.IndooneAuthUI = { showLogin, showSignup, showForgotPassword, close };
