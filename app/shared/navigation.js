...
const MAIN_SVG_ICONS = {
  ...
  sort: '<path d="M7 5v14M4 8l3-3 3 3M17 19V5M14 16l3 3 3-3"></path>',
  empty: '<path d="M4 12h16M12 4v16"></path>',
  pair: '<path d="M8.5 8.5 6.7 10.3a4 4 0 0 0 5.7 5.7l1.8-1.8M15.5 15.5l1.8-1.8a4 4 0 0 0-5.7-5.7L9.8 9.8"></path><path d="m9.3 14.7 5.4-5.4"></path>',
  connect: '<path d="M5 12h14M8 7l-5 5 5 5M16 7l5 5-5 5"></path>',
  devices: '<rect x="3" y="5" width="13" height="10" rx="1.5"></rect><path d="M7 19h5M9.5 15v4M19 9h2v10h-7V9h2M16.5 11h3"></path>',
  status: '<path d="M4 17a11.5 11.5 0 0 1 16 0M7 14a7.4 7.4 0 0 1 10 0M10 11a3.4 3.4 0 0 1 4 0"></path>',
  phone: '<rect x="6.5" y="3.5" width="11" height="17" rx="2"></rect><path d="M10 17.5h4"></path>',
  laptop: '<rect x="5" y="5" width="14" height="10" rx="1.5"></rect><path d="M3 18h18M9 18l-1 2h8l-1-2"></path>',
  back: '<path d="m14 5-7 7 7 7"></path>',
  next: '<path d="m10 5 7 7-7 7"></path>'
};

function makeMainSvg(type, className = 'ui-svg-icon') {
...
function replaceTextNodeIcon(element, iconType) {
  if (!element || element.querySelector('.ui-svg-icon')) return;
  ...
  if (textNodes.length) textNodes[0].textContent = textNodes[0].textContent.replace(/^[\s\u2302\u2315\u2300\u2195\u2194\u2303\u203a\u2039\u2302\u2022]+/u, '').trimStart();
}

function applyMainSvgIcons() {
  const menu = document.getElementById('menuBtn');
  if (menu && !menu.querySelector('.ui-svg-icon')) {
    const icon = makeMainSvg('menu', 'ui-svg-icon topbar-svg-icon');
    menu.textContent = '';
    menu.appendChild(icon);
  }

  const search = document.getElementById('searchBtn');
  if (search && !search.querySelector('.ui-svg-icon')) {
    const icon = makeMainSvg('search', 'ui-svg-icon topbar-svg-icon');
    search.textContent = '';
    search.appendChild(icon);
  }

  const searchBoxIcon = document.querySelector('.search-box > span:first-child');
  if (searchBoxIcon && !searchBoxIcon.querySelector('.ui-svg-icon')) {
    searchBoxIcon.textContent = '';
    searchBoxIcon.appendChild(makeMainSvg('search'));
  }

  const sort = document.getElementById('sortBtn');
  if (sort && !sort.querySelector('.ui-svg-icon')) {
    sort.textContent = 'Sort ';
    sort.appendChild(makeMainSvg('sort'));
  }

  const empty = document.querySelector('.empty-icon');
  if (empty && !empty.querySelector('.ui-svg-icon')) {
    empty.textContent = '';
    empty.appendChild(makeMainSvg('empty'));
  }

  document.querySelectorAll('[data-connect-action="pair"] strong').forEach(el => {
    if (!el.querySelector('.ui-svg-icon')) {
      el.textContent = '';
      el.appendChild(makeMainSvg('pair'));
    }
  });
  document.querySelectorAll('[data-connect-action="connect"] strong').forEach(el => {
    if (!el.querySelector('.ui-svg-icon')) {
      el.textContent = '';
      el.appendChild(makeMainSvg('connect'));
    }
  });
  document.querySelectorAll('[data-connect-action="devices"] strong').forEach(el => {
    if (!el.querySelector('.ui-svg-icon')) {
      el.textContent = '';
      el.appendChild(makeMainSvg('devices'));
    }
  });
  const status = document.querySelector('.connect-status-icon');
  if (status && !status.querySelector('.ui-svg-icon')) {
    status.textContent = '';
    status.appendChild(makeMainSvg('status'));
  }

  document.querySelectorAll('.connect-back').forEach(el => {
    if (!el.querySelector('.ui-svg-icon')) {
      el.textContent = '';
  ...
  document.querySelectorAll('.connect-choice .choice-icon').forEach((el, index) => {
    if (el.querySelector('.ui-svg-icon')) return;
    el.textContent = '';
    el.appendChild(makeMainSvg(index === 0 ? 'phone' : 'laptop'));
  });

  document.querySelectorAll('.connect-choice .arrow').forEach(el => {
    if (!el.querySelector('.ui-svg-icon')) {
      el.textContent = '';
...
function applySvgIconStyles() {
  if (document.getElementById('svg-icon-runtime-style')) return;
